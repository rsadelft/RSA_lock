...
class GitHub:
    def __init__(self, requests=None, remote=None, io=None, logger=None, branch='master', username='', token='',
                 base64=None):
    ...
    def sha(self):
        result = self.requests.get('%s/commits?per_page=1&sha=%s' % (self.remote, self.branch), logger=self.logger,
    ...
    def download(self, sha=None, destination=None, currentDir='', base=''):
        fileList = self.requests.get('%s/contents/%s?ref=%s' % (self.remote, self.io.path(base, currentDir), sha),
                                     logger=self.logger, headers=self.headers)
        ...
        for file in fileList.json():
            if file['type'] == 'file':
                ...
                self.download(sha=sha, destination=destination, currentDir=self.io.path(currentDir, file['name']),
                              base=base)
